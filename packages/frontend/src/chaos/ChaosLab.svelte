    ...
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-auto">
        {#each containers as container}
          <button class="rounded-lg bg-[var(--pd-content-card-bg)] hover:bg-[var(--pd-content-card-hover-bg)] p-4 transition-colors text-left cursor-pointer w-full"
              ...
              <div class="space-y-2">
                <div class="flex items-center gap-2 text-xs text-[var(--pd-content-text)]">
                  ...
                  <div class="flex-1 h-1.5 rounded-full bg-[var(--pd-input-field-bg)] overflow-hidden">
                    <div class="h-full rounded-full bg-[var(--pd-button-primary-bg)] transition-all" style="width: {Math.min(container.stats.cpuPercent, 100)}%"></div>
                  </div>
