<script lang="ts">
import { Button, NavPage, Dropdown, StatusIcon, Checkbox, Tooltip, NumberInput, ErrorMessage } from '@podman-desktop/ui-svelte';
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 overflow-auto">
        ...
        <div class="space-y-1 max-h-80 overflow-auto">
          {#each containers.filter(c => c.state === 'running') as c}
            <button
              ...
              onclick={() => { selectedContainer = c.id; loadNetworks(); checkIptables(); }}>
      ...
      <div class="rounded-lg bg-[var(--pd-content-card-bg)] p-4 space-y-4">
        ...
        <div>
          ...
          <NumberInput bind:value={autoRestoreSec} minimum={0} maximum={3600} step={10} type="integer" aria-label="Auto-restore seconds" />
        </div>
        ...
        {:else}
          <div class="space-y-2">
            {#each isolations as iso}
              <div class="rounded-md border-l-4 border-[var(--pd-status-starting)] bg-[var(--pd-content-card-hover-bg)] p-3">
