  ...
  <div class="flex flex-col w-full p-5 gap-4">
    ...
    <div class="rounded-lg bg-[var(--pd-content-card-bg)] p-5 space-y-5">
      ...
      <div class="grid grid-cols-2 gap-6">
        <div>
          <span class="block text-xs text-[var(--pd-content-text)] mb-2">CPU Cores Limit ({(cpuPercent / 100).toFixed(2)} cores)</span>
          <NumberInput bind:value={cpuPercent} minimum={1} maximum={1600} step={1} type="integer" aria-label="CPU percent" />
        </div>
        <div>
          <span class="block text-xs text-[var(--pd-content-text)] mb-2">Memory Limit (MB)</span>
          <NumberInput bind:value={memoryMb} minimum={16} maximum={8192} step={16} type="integer" aria-label="Memory MB" />
        </div>
      </div>
